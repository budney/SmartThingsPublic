...
metadata {
	...
	tiles (scale: 2) {
		...
		main "list-energy"
		details(["current1","current","current2","power1","power","power2","energy1","energy","energy2","voltage","energy","cost","lastresetlabel","lastresettime","configure","reset","refresh"])
	}
...
def zwaveEvent(physicalgraph.zwave.commands.multichannelv3.MultiChannelCmdEncap cmd) {
	...
	//log.debug ("Command from endpoint ${cmd.sourceEndPoint}: ${encapsulatedCommand}")
		if (encapsulatedCommand) {
        	def n = cmd.sourceEndPoint
            if (encapsulatedCommand.scale == 0 ) { // kWh
            	newValue = encapsulatedCommand.scaledMeterValue
                formattedValue = newValue
                dispValue = "${formattedValue}"
                log.debug "L${n} Energy: ${formattedValue} kWh"
                sendEvent(name: "energy${n}", value: dispValue as String, unit: "", descriptionText: "L${n} Energy: ${formattedValue} kWh")
            }
            if (encapsulatedCommand.scale == 2 ) { // Watts
                ...
                formattedValue = newValue
                dispValue = "${formattedValue}"
                sendEvent(name: "power${n}", value: dispValue as String, unit: "", descriptionText: "L${n} Power: ${formattedValue} Watts")
            }
            if (encapsulatedCommand.scale == 5 ) { // Amps
                ...
                dispValue = "${formattedValue}"
                sendEvent(name: "current${n}", value: dispValue as String, unit: "", descriptionText: "L${n} Current: ${formattedValue} Amps")
            }
		}
	}
}
...
def configure() {
	...
	//zwave.configurationV1.configurationSet(parameterNumber: 255, size: 1, scaledConfigurationValue: 1),
		...
		zwave.configurationV1.configurationSet(parameterNumber: 3, size: 1, scaledConfigurationValue: 0),
